...
namespace Recyclame.Utils
{
    ...
    internal static class Util
    {
        ...
        {
            return isAscii(s);// s.All(c => c < 127);
        }
        ...
        public static bool isAscii(String s)
        {
            ...
            {
                if ((int)c > 0177) // non-ascii
                    return false;
            }
            return true;
        }

        public static string tripCharacters(String s)
        {
            char[] carray = s.ToCharArray();
            ...
            foreach (char c in carray)
            {
                if ((int)c <= 0177) // non-ascii{
                {
                    if (char.IsWhiteSpace(c) || (c != '-' && c != '%' && c != '¿' && c != '?' && !char.IsSurrogate(c) && !char.IsSymbol(c) && !char.IsControl(c) && !char.IsSeparator(c)))
                    {
                        sb.Append(c);
                    }
                    else
                ...
                {
                    if (!char.IsPunctuation(c) && !char.IsSurrogate(c) && !char.IsSymbol(c) && !char.IsControl(c) && !char.IsSeparator(c))
                    {
                        sb.Append(c);
                    }
                    else
        ...
        [Obsolete]
        internal static string[] ToChunks(string str, int characters)
        {
        ...
        internal static bool Contains(string str, string value, StringComparison comparer)
        {
        ...
        internal static void Raise<T>(EventHandler<T> @event, object sender, T args)
            where T : EventArgs
        {
        ...
        internal static void ThrowIfNull<T>(T data, string name) where T : class
        {
            if (data == null)
        ...
        /// <param name="data">The item to check for nullity.</param>
        /// <remarks>Courtesy of Jon Skeet.</remarks>
        internal static void ThrowIfNull<T>(T data) where T : class
        {
            if (data == null)
        ...
        internal static void ThrowIfNullOrEmpty(string s)
        {
            if (s == null)
                throw new ArgumentNullException();
            if (s == String.Empty)
        ...
        /// <param name="s">The string to check for nullity and emptiness.</param>
        /// <param name="name">The name to use when throwing an exception, if necessary.</param>
        internal static void ThrowIfNullOrEmpty(string s, string name)
        {
            if (s == null)
                throw new ArgumentNullException(name);
            if (s == String.Empty)
        ...
        internal static ushort ReadUInt16(BinaryReader reader, bool bigEndian)
        {
            if (!bigEndian)
        ...
        public static string DecodeWordsSubject(string words)
        {
            if (String.IsNullOrEmpty(words))
                return String.Empty;
            MatchCollection matches = rxDecodeWord.Matches(words);
            ...
            foreach (Match m in matches)
            {
                if (m.Index > LastKnownMatchPos)
                    HandleFillData(decoded, words.Substring(LastKnownMatchPos, m.Index -
                        LastKnownMatchPos));
                string sdecode = DecodeWordSubject(m.Groups[0].Value, ref base64, ref encoding, ref sbdecoded);
                if (!base64)
                {
                    if (sbdecoded.Length > 0)
                    {
                        decoded.Append(encoding.GetString(Util.Base64Decode(sbdecoded.ToString())));
                        sbdecoded.Clear();
                    }
                    decoded.Append(sdecode);
                }
                else
                {
                    sbdecoded.Append(sdecode);
                }
                //decoded.Append(DecodeWordSubject(m.Groups[0].Value, ref base64, ref encoding));
                LastKnownMatchPos = m.Index + m.Length;
            }
            if (base64 && sbdecoded.Length > 0)
            {
                string text = sbdecoded.ToString();
                // decoded.Clear();
                try
                {
                    decoded.Append(encoding.GetString(Util.Base64Decode(text)));
                }
                catch (Exception e)
                {
                    System.Diagnostics.Debug.WriteLine("DECODE SUBJECT EXCEPTION: " + e.ToString());
                }
            }
            else
            {
        ...
        internal static string DecodeWordSubject(string word, ref bool base64, ref Encoding encoding, ref StringBuilder sb)
        {
            if (String.IsNullOrEmpty(word))
                return String.Empty;
            Match m = rxDecodeWord.Match(word);
            if (!m.Success)
                return word;
            encoding = Util.GetEncoding(m.Groups[1].Value);
            string type = m.Groups[2].Value.ToUpper();
            string text = m.Groups[3].Value;
            switch (type)
            {
                ...
                case "B":
                    ...
                    {
                        base64 = true;
                        return text;//encoding.GetString(Util.Base64Decode(text));
                    }
                default:
                    throw new FormatException("Encoding not recognized in encoded word: " + word);
            }
        ...
        internal static Encoding GetEncoding(string name)
        {
            ...
            try
            {
                if (string.Compare(name, "windows-1252", true) == 0)
                {
                    name = "iso-8859-1";
                }
        ...
        {
            Util.ThrowIfNull(uids);//.ThrowIfNull("uids");
            List<uint> lst = new List<uint>(uids);
            if (lst.Count == 0)
            {
            ...
            {
                return UIDSet.toString(UIDSet.createUIDSets(lst.ToArray()));
            }
            /*if (uids.GetEnumerator().Count() == 0)
				throw new ArgumentException("The specified collection is empty.");
...
public class UIDSet
{
    ...
    public UIDSet(uint start, uint uintend)
    {
        this.start = start;
        this.end = uintend;
    }
     ...
     */
    public static UIDSet[] createUIDSets(uint[] msgs)
    {
        List<UIDSet> v = new List<UIDSet>();
        int i, j;
